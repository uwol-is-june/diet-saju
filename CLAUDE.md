...
다시 볼 시점은 **TASK-08(결과 캐싱)** 이다. 캐싱이 저장 계층과 보관 기간 문구를 이미 손대므로
그때 같이 판단하면 비용이 한 번만 든다. 그 전에 넣으면 저장 계층을 두 번 만든다.
...
2026-08-13 에 보드를 비웠다. **취소가 아니라 보류**다. 다시 꺼낼 때 조사를 되풀이하지 않도록
그때까지 확인한 것을 남긴다. 착수하면 `docs/TASK.md` 에 새 번호로 태스크를 만든다.
