...
### 올해 운세 판정 (`lib/saju/yearly.ts`)

`yearly` 유형도 같은 원칙이다. 운세는 LLM 이 가장 즉흥적으로 흐르기 쉬운 영역이라 —
같은 사주에 어떤 날은 "재물이 들어오는 해", 어떤 날은 "지출을 조심할 해" 가 나오면
서비스가 아니라 점괘 뽑기가 된다 — **판정을 코드가 한다.**

- 축은 둘이다. **작용**(세운 오행이 원국의 부족을 채우는가 / 과다에 더하는가 → 보완·가중·중립)과
  **주제**(세운 천간의 십신 그룹 → 경쟁과 독립 / 표현과 생산 / 활동과 결실 / 책임과 압박 /
  학습과 정비). 작용 3단계와 주제 대응표는 **우리 관례**다.
- **월별 운세는 계산하지 않는다.** 프롬프트가 "몇 월에 무엇" 을 쓰지 말라고 두 곳에서 막고
  테스트가 확인한다. 월운을 넣으려면 절입 경계까지 따져야 하는데 지금 범위가 아니다.
- **사건을 예고하지 않는다.** 문구 표는 `yearly.test.ts` 가 금지 어휘(반드시·대박·사고·합격…)로
  훑는다. 면책 고지가 "확정된 사실이나 미래 예측으로 받아들이지 마세요" 를 약속한다.
...
- **유형은 셋이며 모두 유지한다** (2026-08-13 확정).
  `종합 사주 풀이` · `체질·다이어트 풀이` · `올해 운세` 를 `readingType` 으로 지원한다.
  어느 하나를 제거하지 말 것. 새 기능은 **세 유형 모두**에 적용되는지 확인한다.
