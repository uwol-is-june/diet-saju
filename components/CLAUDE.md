...
- 사진(래스터)은 토큰 검사 밖이다. 그래서 **글자를 사진 위에 얹는 것은 스크림이 대비를 보증하는
  자리 둘뿐이고**(판정 콜아웃 · `/` 유형 카드) 거기서도 글자·화살표는 전부 토큰(`on-photo*`)이다.
  나머지 자리에서는 얹지 않는다.
...
| 유형 카드 | `ReadingTypeCard.tsx` | 사진 전면 + 어둠 + 흰 글씨 · 최소 13rem · `/` 와 결과 화면이 공유 |
...
한 줄에 한 장. **사진이 카드를 통째로 덮고 그 위에 어둠 + 흰 글씨**(묻는 것 · 제목 · 설명 ·
조회수)가 얹히고 오른쪽 아래에 원형 화살표. 판정 콜아웃과 같은 꼴이다.

**카드는 `ReadingTypeCard` 가 그리고 결과 화면 맨 아래가 같은 부품을 쓴다** (TASK-115).
이 페이지가 정하는 것은 **목록 구조와 등장 순서**뿐이다 — 카드 안을 여기서 다시 그리지 말 것.
부품에 **`"use client"` 를 붙이지 않고 `lib/counters` 같은 서버 모듈을 끌어들이지 않는다**
(앞은 `/` 의 정적 성질, 뒤는 클라이언트 쪽 호출부가 깨지는 문제다).

- **`/` 에는 클라이언트 컴포넌트가 없다.** 정적(ISR 5분)을 유지하는지 빌드 표로 확인한다.
- **글에 폭 제약이 없다.** 어둠이 카드 전체에 깔리므로 글이 열 폭을 다 쓴다 — 예전 `글 58% +
  사진 42%` 는 두 폭이 서로를 묶어 두던 값이고 **되살리지 말 것**(`birth-input.test.ts` 가
  `w-[NN%]` 가 다시 나타나는 것을 잡는다). 최소 높이는 **콜아웃과 같은 13rem** 이다 — 글 세 줄이
  정하는 높이로는 전면 사진이 띠 한 줄로 보인다.
- **스크림 알파는 최악의 사진(흰 면)에서 계산한다** (`--card-scrim`). hover 는 면 색이 아니라
  **스크림을 깊게 하는 쪽**으로 움직인다 (어두워지는 방향이라 하한이 그대로 산다).
  `tokens.test.ts` 가 콜아웃과 **같은 계산을 두 블록에** 돌린다.
- 글자·화살표는 `on-photo*` 만 쓴다. **위계는 무게와 크기로만** (색은 흰색 하나).
...
- **첫 세 장에만 `priority` 를 준다** (다섯 장 전부에 주면 preload 가 서로를 밀어낸다).
  전면 깔기로 바꾸면서 사진 자산이 190KB → 227KB 로 늘고 LCP 는 그대로였다
  (0.70 → 0.73초 · 2026-08-21 · 프로덕션 빌드 · 390px · DPR 2 · 1.6Mbps · 3회 중앙값).
...
- **사진 위에 글자를 얹을 수 있는 조건은 하나다 — 스크림이 대비를 보증할 때.** 사진 안의 색은
  ...
  정하고 그 계산을 테스트가 다시 하게 한다 (지금 그렇게 하는 곳은 판정 콜아웃 · `/` 유형 카드 ·
  공유 카드 셋이고, 셋이 **같은 알파**를 쓴다).
...
| 자리 | 컴포넌트 | 모양 |
| --- | --- | --- |
| `/` 카드 전면 | `ReadingCardPhoto` | 카드 폭 · `.card-cover` 의 어둠 아래 · 크롭 한 값(`50% 50%`) |
...
- 마스크가 남은 곳은 히어로 하나다 (`globals.css` 의 `.hero-photo` · 아래·좌우로). `-webkit-`
  접두사를 함께 둔다. **레이어 하나로 끝낸다** — 두 겹 쌓으면 `mask-composite` 가 필요하고
  브라우저에 따라 사진이 통째로 사라진다. `/` 카드는 마스크가 아니라 **스크림**이다
  (`.card-cover`), 그래서 그 위에 글자를 얹을 수 있다.
- **원본은 480×480 이고 `fetch-card-photos.mjs` 의 `SIZE` 가 단일 소스다.** 전면 깔기라 카드 폭보다
  작지만 스크림 아래 장식이라 그대로 둔다 — 올릴 때는 **컴포넌트의 `SLOT` 과 함께** 올린다.
...
- **근거 카드에 묶음 머리를 두지 않는다.** 같은 모양의 접힌 카드 넷이 줄지어 있으면 글자로
  이름을 달지 않아도 한 덩어리로 읽힌다. 그래서 **카드 제목이 `h2`** 다 — 위에 묶음 제목이
  없으므로 `h3` 로 두면 화면에 없는 단계를 건너뛴다. **크기(`title-sm title-bold`)는 그대로다.**
- 좋아요와 공유는 `space-y-2` 한 묶음. `OtherReadingLinks` 는 따라오지 않는다 (그것은 화면을 다 본
  뒤의 동선이라 `SajuForm` 맨 아래가 제자리다). 셋 다 `!streaming` 이다.
- **`OtherReadingLinks` 는 `/` 와 같은 부품(`ReadingTypeCard`)으로 그린다** (TASK-115) — 같은 일을
  하는 자리가 두 모습이면 규격도 두 벌이 된다. 상자를 두르지 않고 제목 + 카드 목록만 둔다.
  카드 넷이 붙어 800px 쯤 늘어나지만 **낮은 변형을 따로 만들지 않는다** — 규격을 정하는 곳은
  한 파일이어야 한다.
  - **조회수를 넘기지 않는다** (`counts` 없음). 여기는 클라이언트 트리라 서버에서 읽은 값이 없고
    클라이언트에서 fetch 하면 풀이 한 번에 요청이 유형 수만큼 붙는다.
  - **`priority` 도 주지 않는다** (스크롤 아래라 LCP 가 아니다). **`anim-card-rise` 도 걸지 않는다** —
    거기서는 스크롤로 내려가 만나는 자리라 지연이 빈 카드를 보여 준다.
...
- **무엇을 띄울지는 `lib/reading/verdict.ts` 가 정하고 컴포넌트는 그리기만 한다** —
  **공유 카드가 같은 값을 읽는다**(`verdictOf`). 표를 컴포넌트로 되돌리면 화면과 저장된
  이미지가 다른 말을 한다.
...
- **그 한 줄은 판정 축의 이름을 대지 않고 라벨의 뜻을 생활어로 말한다.** 모르는 말로 출처를 대면
  ...
  않는다**(글 폭이 좁아 줄이 자란다 · 용어 풀이는 본문의 일이다). **판정의 범위를 한정하는 자리는
  이제 `ResultView` 하단 고정 문구 하나다** (눈썹 줄은 주제를 말하는 줄이 되고 `내 사주` 묶음
  머리는 없어졌다) — `오락·참고용` 과 `AI 가 작성` 이 그 일을 하고 있으므로 **그 문구를 지우면
  범위를 말하는 자리가 화면에 하나도 남지 않는다.**
- **눈썹 줄은 유형마다 다르고 그 유형이 답하는 질문을 말한다** (`EYEBROW` = `Record<ReadingType,
  string>` · 삼항·인덱스 시그니처 금지). 유형 이름을 그대로 되풀이하므로 페이지 제목과 같은 말이
  한 화면에 두 번 나오는데 **그대로 간다** — 제목은 화면의 이름이고 눈썹 줄은 아래 라벨을 여는
  말이다. **판정 축의 이름을 대지 않고**(근거 줄과 같은 규칙) 390px `text-xs` 에서 한 줄이다.
...
- **"저장하지 않습니다" 를 화면에서 말하는 자리는 둘이다** — `FirstVisitNotice`(`/reading/*` 에만,
  정보를 넣기 직전에 보여야 한다)와 **모든 화면의 푸터**. 폼 제출 버튼 아래에도 있었지만 같은
  화면에서 같은 약속을 두 번 하는 것이라 지웠다 (TASK-118). **`FirstVisitNotice` 는 닫으면
  사라지므로 두 번째 방문자에게는 푸터 한 줄이 마지막 자리다** — 그것까지 지우면 약속을
  말하는 자리가 화면에 없다 (`app/privacy/page.tsx` 는 약속의 본체이지 화면의 자리가 아니다).
...
**카드는 판정 한 줄이다** (TASK-116) — 위쪽은 화면 콜아웃과 같은 꼴(**사진 전면 + 어둠 + 흰
글씨**로 눈썹 줄 · 라벨 · 근거 한 줄)이고 아래쪽에 띠·계절과 오행 막대가 온다.

- **간지를 한 자도 싣지 않는다.** 네 기둥이 정해지면 60년 주기 안에서 생년월일시가 거의
  유일하게 특정되고 대운 간지가 나이까지 좁힌다 — **기준은 날짜 문자열이 아니라 역산
  가능성**이다. `card-model.test.ts` 가 간지·나이·날짜를 전부 막는다.
- **값을 카드가 스스로 고르지 않는다** — `verdictOf`(`lib/reading/verdict.ts`)를 부른다.
- **어둠의 알파는 화면과 같은 값**(`:root` 의 `--verdict-scrim`)을 `readPalette` 로 읽어 쓴다.
  캔버스는 CSS 를 읽지 못하므로 여기서 숫자를 새로 정하면 대비 근거가 두 벌이 된다.
- 사진은 우리 도메인 자산이라 캔버스가 오염되지 않는다(`toBlob` 이 그대로 된다).
  **못 받으면 연한 면 꼴로 떨어진다** — 사진 없는 내부 유형이 쓰는 그 꼴이다.
- **넘치는 사진은 잘라낸다** (`ctx.clip`). 어둠은 슬롯 안에만 깔리므로 잘라내지 않으면
  넘친 사진이 아래 근거 자리를 덮는다.
