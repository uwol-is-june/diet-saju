...
/**
 * 만세력 계산 기준 — 출생시각 보정 · 자시 기준.
 *
 * **폼이 아니라 결과 화면이다.** 이 설정을 만지고 싶어지는 순간은 폼을 채울 때가 아니라
 * **결과를 보고 다른 곳과 다를 때**다 — 근거는 읽고 난 뒤에 확인하는 것이다.
 * **없앤 것이 아니라 옮긴 것이다**: 23시대 출생자는 자시 기준에 따라 일주가 통째로 바뀌고,
 * 다른 만세력과 결과가 다를 때 원인을 짚는 수단이 이것뿐이다. **폼으로 되돌리지 말 것.**
 *
 * **`고급` 이라는 낱말을 쓰지 않는다** — 열면 안 되는 것처럼 읽히는데 23시대 출생자에게는
 * 필수 항목이다. 여는 이유는 접힌 줄의 `note`(실제 적용된 보정량)가 말한다.
 *
 * **바꾸면 자동으로 다시 받지 않는다.** 값이 바뀌면 화면의 풀이·원국이 옛 기준의 것이 되므로
 * 그 사실을 알리고 `이 기준으로 다시 보기` 버튼을 낸다 — 즉시 재제출하면 둘을 함께 바꿀 때
 * 요청이 두 번 나가고, 무료 등급의 병목은 토큰이 아니라 **요청 수**다.
 *
 * 캐시는 손댈 것이 없다 — `reading-cache.ts` 가 입력 스냅샷 전체를 훑으므로 두 값이 바뀌면
 * 통째로 비워진다. **손으로 키에 더하지 말 것.**
 */
export function CalculationBasis({
  ...
  /**
   * **시각 미상이면 `pillars.ts` 가 보정을 강제로 끈다.** 그때 고른 값과 쓰인 값이 다르므로
   * 화면은 **실제로 쓰인 것**과 비교해야 한다 — 안 그러면 바뀐 것이 없는데도 "다시 보기" 가
   * 계속 떠 있다.
   */
...
export function describeCalculationBasis(correction: TimeCorrectionInfo): string {
  /**
   * **시각 미상이면 두 값 다 쓰이지 않는다.** 그때 `자정 기준` 이라고 적으면 쓰이지도 않은
   * 값을 적용된 것처럼 말하게 된다 — 이 줄은 고른 값이 아니라 **적용된 값**을 말하는 자리다.
   */
