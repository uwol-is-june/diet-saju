 ...
 * ## 이 그림은 이 크기에서만 산다
 *
 * 예전에는 `/` 의 56px 썸네일(`thumbnails/ReadingThumbnail.tsx`)이 따로 있었고 **같은 그림을
 * 키워 쓰지 않는다** 가 여기 적혀 있었다. 그 썸네일은 TASK-86 에서 사진으로 바뀌어 없어졌지만
 * **판단은 그대로 유효하다** — 작은 크기에서는 도형 수를 줄이고 굵기를 키우는 쪽이 이기고
 * (`decade` 를 눈금 다섯 칸으로 그렸다가 바코드로 읽혀 고쳤다), 그런 그림을 키우면 반대로
 * 성기고 밋밋해진다. 여기는 표정과 볼터치가 들어가는 자리다.
 *
 * ## 손그림 결이란 무엇인가
 *
