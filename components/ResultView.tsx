  ...
  return (
    <div className="space-y-6">
      ...
      */}
      ...
      {/*
        근거 카드 넷 (TASK-61 · 82 · 112). **풀이 아래에 둔다** — 사람들이 보러 온 것은
        풀이이고 도식은 그것이 무엇에서 나왔는지 보여주는 근거다.

        **묶음 머리를 두지 않는다** (TASK-112). `내 사주` 제목과 `이 풀이는 아래 값에서
        나왔습니다` 한 줄이 여기 있었는데, 접힌 카드 넷이 이미 같은 모양으로 줄지어 있어
        묶음이 글자로 이름을 달지 않아도 한 덩어리로 읽힌다. 머리가 없어졌으므로 **카드
        제목이 `h2` 다** — 제목 단계가 건너뛰어지지 않게 한 것이고 근거 카드가 풀이 절과
        같은 단계로 제목 목차에 나란히 선다.
      */}
...
function FoldCard({
  ...
  return (
    ...
    >
      {/*
        제목은 **`h2`** 다 (TASK-112). 예전에는 `내 사주` 묶음 머리(h2) 아래 단계라 `h3`
        였는데 그 머리를 지웠으므로, `h3` 로 남기면 화면에 없는 단계를 건너뛰어 스크린리더
        목차가 어긋난다. 넷이 같은 단계여야 제목 탐색으로 근거 카드를 훑을 수 있다.
        **크기는 그대로다** — 단계와 규격은 다른 이야기다 (`title-sm title-bold` 는
        `ReadingSections` 의 절 제목과 같은 값이다).
      */}
      <summary className="flex items-center gap-x-3 gap-y-1 p-5 sm:p-6">
        <h2 className="title-sm title-bold">{title}</h2>
