 ...
 * 요청 결과 계측 (TASK-13).
 *
 * ## 상태 코드만 세면 실패율이 0 으로 보인다
 *
 * `/api/saju` 는 원국을 먼저 보내려고 응답을 일찍 시작한다. 그래서 **Gemini 실패는 200 본문
 * 안의 `error` 이벤트로** 나간다 (배경은 `app/api/saju/route.ts`). 상태 코드로 집계하면
 * 쿼터 소진도 안전정책 차단도 전부 200 으로 세어져 "실패 0건" 이 된다.
 *
 * 그래서 결과를 상태 코드가 아니라 **`SajuOutcome` 으로** 센다. `reading_failed` 가 그
 * 200-속-실패에 해당한다.
 *
 * ## 개인정보를 남기지 않는다
 *
 * 개인정보 처리방침이 생년월일을 "로그에도 기록하지 않는다" 고 약속한다. 여기 실리는 것은
 * 전부 **범주형 파생값**이며, `METRIC_FIELDS` 허용 목록에 있는 키만 직렬화한다.
 * 타입을 우회해 필드를 더 넣어도 조용히 버려지고, `observability.test.ts` 가 그것을 검사한다.
 * 성별은 값이 아니라 "지정했는지" 만 남긴다 (대운 산출 가능 여부를 보기 위한 것).
 *
 * ## 어디로 나가는가
 *
