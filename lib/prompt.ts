...
구체적인 식단 이름·단식·칼로리·목표 체중·기간을 쓰지 말 것. 순서와 습관까지만 쓴다.
실행 온도·시간대는 "한열" 항목을 쓰되 여기서 길게 쓰지 말고 아래 두 절에 넘긴다.`,
  eating: `"관리 축" 과 "한열" 의 식습관 항목에서 3~4가지를 뽑아 쓴다. 판정에 없는 것을 지어내지 않는다.
**위 절에서 정한 접근 순서와 어긋나지 않게** 쓴다 (예: 리듬 고정 우선이면 메뉴보다 시각을 먼저).
...
- 조리 방식과 음식 온도는 "한열" 항목을 따른다. 재료(오행)와 조리(한열)는 다른 층이다.
...
식습관·움직임·접근 순서 항목은 문장을 그대로 베끼지 말고 근거로 삼아 다시 쓰세요.`,
};
