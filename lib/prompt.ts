...
실행 지시로 바꾸지 말 것.
원인을 확정하지 말고 "~쪽에서 먼저 드러나는 결" 처럼 조건과 경향으로 쓴다.`,
...
마지막에 이 풀이가 원인을 확정한 것이 아니라 사주에서 읽히는 경향이라는 것을 한 문장으로 밝힌다.
여기서도 방법을 처방하지 않는다.`,
...
const SUMMARY_INSTRUCTION: Record<ReadingType, string> = {
  ...
  "gain-cause":
    "3~4문장으로 이 사주에서 살이 붙는 결을 요약. **판정된 걸리는 지점과 살이 붙는 패턴을 여기서 밝힌다.** 원인을 단정하지 말고 경향으로 쓴다.",
  "diet-method":
...
- 체중·건강 문제는 전문가와 상의하도록 권하는 한 문장을 마지막에 덧붙이세요.`,
  ...
  "gain-cause": `
# 표현 규칙 (반드시 지킬 것)
- **원인을 확정하지 않는다.** "당신은 ~때문에 찝니다" 처럼 단정하지 말고 "~할 때 붙기 쉬운
  결입니다" 처럼 조건과 경향으로 쓰세요. 제목이 이유를 묻고 있어도 답은 확정된 원인이 아니라
  이 사주에서 읽히는 결입니다.
