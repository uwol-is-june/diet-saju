...
읽어 낸 결 자체는 단정해서 쓰되, 마지막에 이 풀이가 **사주에서 읽은 결이지 몸에서 실제로
일어나는 일을 확인한 것이 아니라는 것**을 한 문장으로 밝힌다.
이때 "진단"·"질환" 같은 의학 낱말을 부정문으로라도 쓰지 말 것 — 위 표현 규칙이 막는 낱말이다.
여기서도 방법을 처방하지 않는다.
분량: 10문장 안팎(약 550자).`,
...
들어가는지 **하루·한 주의 장면을 두 가지 이상** 보여준다 (출근 전, 점심 뒤, 주말 아침처럼).
**장면으로 열고 근거는 그 뒤에** 붙인다.
