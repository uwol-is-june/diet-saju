 ...
 * 본문 강조. 만드는 주체가 **둘**이다.
 *
 * 1. **프롬프트가 본류다** — 어디가 핵심인지 아는 것은 문장을 쓰는 쪽뿐이다.
 *    `SYSTEM_INSTRUCTION` 이 절마다 한두 곳까지로 상한을 준다.
 * 2. **코드가 아는 것은 판정 라벨뿐이다** (이 파일). 라벨은 코드가 정한 값이라 매번 같은
 *    자리에서 확실히 굵어진다.
 *
 * 스타일은 `globals.css` 의 `.reading strong` 이 정한다. **색을 따로 주지 않는다** —
 * `brand-ink` 를 얹으면 강조가 링크처럼 읽힌다.
 */
...
/**
 * 아직 닫히지 않은 `**` 를 지운다. 스트리밍 중에는 `**식욕형` 까지만 도착한 순간이 반드시
 * 있고 그때 `react-markdown` 은 별표를 **글자 그대로** 그린다.
 *
