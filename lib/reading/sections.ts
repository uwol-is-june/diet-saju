 ...
 * 풀이의 섹션 계약.
 *
 * **JSON 구조화 출력(`responseSchema`)을 쓰지 않는다** — 완성 전까지 파싱이 불가능해서
 * 점진 렌더가 죽는다(첫 글자가 1초대에 보이는 것을 4초로 바꾸는 대가다). 대신 모델이
 * 마크다운을 흘려보내고 `## 제목` 이 도착할 때마다 그 앞 섹션을 완결로 보고 렌더한다.
 *
 * 대가는 형식을 **강제**하는 게 아니라 **부탁**한다는 것이다. 그래서 파서는 어긋난 출력을
 * 버리지 않는다 — `id: null` 로 담고, 아무것도 못 잡으면 `recognized: false` 로 원문
 * 폴백으로 넘긴다. **모델 출력을 잃는 경로는 없다.**
 *
 * **이 파일은 클라이언트에서도 import 된다.** 그래서 `server-only` 가 없고 **프롬프트
 * 지시문도 두지 않는다** — 계약(제목·순서·강조)만 담고 지시문은 `lib/prompt.ts` 가 id 로
 * 붙인다. 제목 문자열이 이 파일 하나에만 있으므로 프롬프트와 렌더러가 어긋날 수 없다.
 */
...
export const SECTION_SPECS: Record<ReadingType, readonly ReadingSectionSpec[]> = {
  ...
  /**
   * 다이어트 계열은 **묻는 것으로 갈라져 있다.** **섹션 id 를 공유하지 않는다** —
   * `SECTION_INSTRUCTION` 이 id 로 매기므로 공유하면 맥락이 다른 유형이 한 지시문을 떠안는다.
   */
  diet: [
    { id: "summary", title: "한눈에 보기", emphasis: "summary" },
  ...
  /**
   * 원인 유형. `diet` 의 `gain-pattern` 과 **같은 판정을 다른 각도로 쓴다.**
   * 그쪽에서 절을 떼어 오지 않은 것은 **`diet` 가 "종합" 이라는 약속** 때문이고, 대신 두
   * 지시문이 서로 다른 것을 요구하게 해서 같은 문장이 두 번 나오지 않게 막는다.
   *
   * **실행 방법 절이 없다** — 무엇을 어떻게 할지는 `diet-method` 몫이다.
   */
  "gain-cause": [
    { id: "summary", title: "한눈에 보기", emphasis: "summary" },
  ...
  /**
   * 운동 유형. `diet-method` 가 **무엇을 어떤 순서로** 라면 이쪽은 **어떤 운동을** 이다.
   *
   * **`diet-method` 의 `movement` 절을 떼어 오지 않았다** (방법 유형에서 움직임을 빼면
   * 반쪽이 된다). 대신 **두 지침이 서로 다른 것을 요구한다** — 그쪽은 종류까지, 여기는
   * 종목·강도·시간대·주의까지. `prompt.test.ts` 가 대조한다.
   */
  /**
   * 식단 유형. **`diet-method` 의 `eating` 절을 떼어 오지 않았다.** 대신 층을 나눈다 —
   * 그쪽은 **순서와 시각**, 여기는 **재료 범주·조리·온도**다.
   *
   * **제목은 모델이 재현하기 쉬운 낱말로 고른다.** 과다 오행 절을 **"덜어낸다"·"끊는다" 로
   * 쓰지 않는다** — 알레르기·지병을 모르므로 "더 늘리지 않기" 까지다.
   */
  "diet-food": [
    { id: "summary", title: "한눈에 보기", emphasis: "summary" },
  ...
  /**
   * 시기 유형. `diet` 의 `year-flow`(올해)와 층이 다르지만 **둘 다 시간을 말하므로 겹칠
   * 위험이 크다** — id 를 나누는 것으로 그치지 않고 지시문이 서로 다른 것을 요구하게 했다
   * (`prompt.test.ts` 가 대조한다).
   *
   * **다음 10년을 판정하지 않는다** — 절은 지금 구간과 **직전 구간과의 차이**까지다.
   */
  decade: [
    { id: "summary", title: "한눈에 보기", emphasis: "summary" },
...
/**
 * 섹션 제목 앞에 붙는 아이콘. **화면에서만 붙는다.**
 *
 * **`title` 에 섞지 말 것.** 계약을 `## 🌿 오행으로 본 체질` 로 바꾸면 모델이 그 이모지까지
 * 정확히 재현해야 파싱되고, 어긋나면 그 절이 `id: null` 로 떨어져 **아이콘도 강조도 함께
 * 사라진다.** 형식 강제 수단이 부탁뿐인데 부탁할 것을 늘리는 셈이다.
 *
 * **`Record` 여야 한다** — 선택 필드로 두면 새 섹션이 조용히 아이콘 없이 나간다.
 *
 * **흑백 폴백이 있는 흔한 이모지만** 쓰고, 이형자 선택자가 필요한 문자(`⚖️`)는 선택자를
 * 붙인 채로 저장한다(빼면 플랫폼에 따라 글자 모양이 된다).
 ...
 *
 * `### 소제목` 이 오는 중이면 `##` 까지 받은 순간에는 섹션 제목과 구별할 수 없어 빈 카드가
 * 번쩍인다. 제목 글자가 한 자라도 오면 `#` 개수는 확정이므로 **해시만 있는 마지막 줄만**
 * 보류한다.
 */
 ...
 * 도착한 마크다운을 섹션으로 가른다.
 *
 * **누적 문자열 전체를 매번 다시 파싱한다** — 증분 상태를 들면 조각 경계 버그를 다 떠안는다.
 *
