/**
 * 원국에서 파생되는 해석 근거 계산. 순수 함수 모듈 — 라이브러리도 I/O 도 없다.
 *
 * **고전 규칙과 우리 관례를 구분한다.** 코드가 만드는 "사실" 이 실제로는 임의의 선택이면
 * 그것을 사실처럼 내보내면 안 된다.
 *
 * - **고전**: 왕상휴수사, 지지 본기, 대운 순행/역행, 대운·세운 간지
 * - **우리 관례**: 오행 점수의 배수, 신강/신약 4단계 표현 (`docs/saju-validation.md` 에도 남겼다)
 */
import {
  JI_KO,
  fromSexagenary,
...
/**
 * **우리 관례**: 왕상휴수사 상태를 오행 점수의 배수로 쓴다. 단순 개수는 계절을 무시하고,
 * 위치별 가중치를 임의로 만드는 것보다 고전 등급을 배수로 삼는 편이 근거가 한 곳에 모인다.
 * **배수 값 자체는 우리가 정한 것이다.**
 */
 ...
 *
 * **득지·득세만 통근(通根)으로 본다** — 일지·년지·시지는 본기가 아니라 지장간 전체로
 * 판단한다(`isRootedIn`). 본기만 보면 숨은 뿌리를 놓쳐 신약 쪽으로 치우친다.
 *
 * **득령은 본기 기준을 그대로 둔다.** 월령은 그 달을 지배하는 기운이라 숨은 글자가 아니고,
 * 여기까지 열면 세 기준이 전부 통근 검사가 되어 판정이 신강 쪽으로 쏠린다
 * (분포표는 `docs/saju-validation.md` 3-2).
 *
 * 기준이 `isSupportingSipsin` 그대로라 **판정이 약해지는 방향으로는 움직이지 않는다.**
 *
 * **표시용 지지 십신(`jiSipsin`)은 본기 기준을 유지한다** — 통근은 판정에만 쓴다. 그래서
 * "일지 편재인데 득지 ○" 가 나올 수 있어 `rooted` 로 근거를 함께 낸다.
 *
...
export interface StrengthAnalysis {
   ...
   * **지지는 전부 통근 기준이다** — 세력을 세는 값이라 "뿌리가 있는가" 로 본다.
   * 그래서 득령이 아니오인 월지가 여기서는 돕는 글자로 잡힐 수 있다.
   */
  supportingChars: number;
  totalChars: number;
  /**
   * 일간이 통근한 자리 — 자리 순서(년→월→일→시)를 지킨다.
   * 지지 십신 표시가 본기 기준이라 통근한 자리가 재성·관성으로 보일 수 있어, 근거가
   * 어긋나 보이지 않도록 어느 자리에 뿌리가 섰는지 함께 낸다.
   */
  rooted: string[];
}
...
export interface DaeunPeriod {
  ...
  ohaeng: string;
  /**
   * 천간·지지의 60갑자 인덱스. `ohaeng` 은 표시용 문자열이라 판정에 쓸 수 없다 —
   * **문자열을 쪼개 쓰지 말고** 이 인덱스로 `ganOhaeng`/`jiOhaeng` 을 부른다.
   */
  gan: number;
  ji: number;
  /** 대운 천간의 십신 */
 ...
 * **우리 관례**: 반올림하고 최소 1세로 둔다 (개월로 환산하는 유파도 있다).
 * 근거가 되는 `daysToJeol` 을 함께 내보내므로 검산할 수 있다.
 */
 ...
 * 입춘 이전은 전년도 간지지만 여기서는 **연 단위 표시**가 목적이라 해당 연도를 그대로 쓴다.
 * 절입 경계까지 따지는 것은 지금 범위가 아니다.
 */
