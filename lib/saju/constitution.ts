 ...
 *
 * 근거는 `GAIN_PATTERN_NOTE` 의 각 문구다. 비겁·인성 쪽은 "활동량이 줄면 붙는" 쪽이고,
 * 식상·재성·관성 쪽은 먹는 양·시각·상황에서 붙는 쪽이다.
 */
 ...
 *
 * **수치는 계속 막는다.** 종목 이름과 강도의 감각까지만 열려 있고 횟수·세트·분·심박수·기간은
 ...
 *
 * ## `short` 는 화면에 크게 뜨는 이름이다 (TASK-102)
 *
 * `diet-food` 콜아웃 라벨과 공유 카드 칩이 함께 쓴다. 예전에는 그 자리에 **오행 이름**이
 * 그대로 나가서(`금 계열을 곁들이기`) 공개 다섯 유형 중 **이것만 오행을 아는 사람만 읽을
 * 수 있었다.**
 *
 * - **`groups` 에서 골라 쓰지 않는다.** `무와 버섯` 은 `groups[0]`·`groups[1]` 을 섞은
 *   값이라 코드로 뽑을 수 없다. 표기용 값이므로 손으로 적되 `Record` 를 유지한다 —
 *   오행이 다섯이므로 빠지면 컴파일이 잡는다.
 * - **맛(매운맛·쓴맛)을 여기 올리지 않는다.** 한열이 `열` 인 사람 화면에서
 *   `THERMAL_GUIDE.열` 의 "맵고 뜨거운 음식과 술을 줄이기" 와 정면으로 부딪힌다 —
 *   **재료는 오행, 조리·온도는 한열**이라는 층 구분이 라벨에서 무너지는 자리다.
 *   색만 쓰는 안(`흰빛 재료`)도 버렸다: 우리가 지어낸 말이라 한국어로 어색하다.
 * - 위의 "지키는 선" 이 그대로 적용된다. 테스트가 `groups` 와 같은 검사를 건다.
 */
 ...
 * 단정해도 되지만 단정하는 대상은 **이 사주에서 읽어 낸 판정**이지 몸에서 실제로
 * 일어나는 일이 아니다. `~해서 찐 살` 은 몸의 인과를 주장하는 문장이라 (ii) 쪽에 걸린다.
 *
 * 그래서 형태를 **"~할 때 붙는 성향"** 으로 고정했다 — 서술은 단정형이고(무르게 만드는
 * 어미가 없다), 주장하는 것은 그 성향이 드러나는 **자리와 상황**까지다. `~할 때` 는
 * 언제인지를 말할 뿐 원인을 말하지 않는다. `constitution.test.ts` 가 `때문에`·`~해서`
 * 류 인과 어미와 hedge 어미를 **양쪽에서** 막는다.
 *
 * **`결` 을 쓰지 않는다** (TASK-72). 그 낱말이 맡던 일((i)판정은 단정하고 (ii)몸의 인과로는
 * 넘어가지 않는 자리)은 위 `~할 때` 형태가 그대로 한다 — 없앤 것은 낱말이지 경계가 아니다.
 */
