...
export const DECADE_EFFECT_NOTE: Record<YearlyEffect, string> = {
  ...
  중립:
    "이 10년 동안 들어오는 기운이 원국의 넘치는 쪽도 얇은 쪽도 크게 건드리지 않는다. 바깥 흐름보다 스스로 정해 둔 생활의 결이 그대로 드러나는 구간이다.",
};
...
export const SHIFT_NOTE: Record<DecadeShift, string> = {
  ...
  전환: "직전 10년과 결이 바뀌었다. 몸이 반응하는 자리가 옮겨 가는 구간이라, 하던 방식을 한 번 점검할 만하다.",
  유지: "직전 10년과 결이 크게 다르지 않다. 지금까지의 방식이 여전히 같은 자리에서 통한다.",
};
