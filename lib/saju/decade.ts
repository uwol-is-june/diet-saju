/**
 * 대운(10년) 판정. 순수 함수 모듈 — **판정은 여기서 하고 LLM 은 서술만 한다**(시간 이야기가
 * 가장 즉흥적으로 흐르기 쉽다).
 *
 ...
 *
 * **직전 구간과의 비교는 새 관례가 아니다** — 같은 규칙을 한 번 더 적용한 것이다.
 * **다음 구간은 판정하지 않는다**(아직 오지 않은 시간을 말하면 예측이 된다).
 *
 * **단정적 예언을 하지 않는다.** 아래 문구는 사용자에게 인용되고 `decade.test.ts` 가 사건
 * 예고·단정 어휘를 훑는다. `app/disclaimer/page.tsx` 도 "정해진 시기가 아니다" 를 약속한다.
 */
...
/**
 * 지금 나이가 속한 대운을 찾는다. **여기 한 곳에서만 정한다** — 흩어지면 화면·카드·풀이가
 * 서로 다른 대운을 가리킨다.
 *
 * `currentAge` 는 `chart.seun[0]?.age` 이며 **세는나이** 관례다.
 */
