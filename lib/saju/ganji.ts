 ...
 * 마지막 항목이 정기(=본기)이고 `jiBongi` 가 그것을 읽는다. **본기를 따로 적지 않는다.**
 *
 * **일수 가중치를 쓰지 않는다** — 일수 배분은 유파마다 다르지만 "어느 천간이 들었는가" 는
 * 이견이 거의 없다. 있나 없나만 보면 우리 관례를 늘리지 않는다.
 *
 * 판본 차이가 있는 자리 (`docs/saju-validation.md` 3-2): 오(午)의 중기 기(己)와 해(亥)의
 * 여기 무(戊)는 넣는 판본을, 신(申)의 여기는 무(戊)를 따랐다.
 */
...
/**
 * 통근(通根) — 일간이 그 지지에 뿌리를 두는가. 지장간 **전체**에 일간을 돕는
 * 천간(비겁·인성)이 하나라도 있으면 참이다.
 *
 * `isSupportingSipsin` 과 같은 기준을 쓰므로 **본기가 이미 돕는 지지는 반드시 여기서도
 * 참이다** — 판정이 약해지는 방향으로는 움직이지 않는다.
 */
