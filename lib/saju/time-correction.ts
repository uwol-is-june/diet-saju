 ...
 * 출생 시각 보정.
 *
 * 사주에서 "몇 시에 태어났나"는 시계가 아니라 **하늘**을 기준으로 판정해야 한다.
 * 시계시(한국 표준시)는 다음 세 가지 이유로 실제 태양 위치와 어긋난다.
 *
 * 1. 경도차 — 한국 표준시는 동경 135°(또는 127.5°) 기준이지만 서울은 약 127°다.
 * 2. 표준자오선 변경 — 1954-03-21 부터 1961-08-09 까지는 동경 127.5°(UTC+8:30)를 썼다.
 * 3. 서머타임 — 1948~51, 1955~60, 1987~88 에 시계를 1시간 앞당겼다.
 *
 * 2·3 은 직접 표를 만들지 않고 **Node 내장 IANA tz 데이터베이스**(Asia/Seoul)에서 가져온다.
 * 실측으로 1954-03-21 / 1961-08-10 전환과 모든 서머타임 구간이 정확히 반영되어 있음을 확인했다
 * (docs/saju-validation.md 참고). 손으로 옮긴 표보다 정확하고, tzdata 갱신을 자동으로 따른다.
 *
 * 순수 계산 모듈 — 외부 I/O 없음.
 */
...
/**
 * 벽시계 시각(그 지역 사람이 시계에서 읽은 값)을 절대 시각으로 바꾼다.
 *
 * 오프셋이 시각에 의존하므로 한 번 추정한 뒤 재확인한다.
 * (서머타임 전환 경계에서 첫 추정이 틀릴 수 있다)
 */
